...
const promoMock = {
    ...
    calloutMsg: 'follow the white rabbit',
    custom: {
...
describe('Product promotions decorator', function () {
    ...
    it('Promotions field with a promo loaded', function () {
        let product = {};

        promotions(product, mockCollectionOfPromos);
        const promotionsLoaded = product.promotions;
        const promo = promotionsLoaded[0];
        assert.equal(promo.id, 'hello Mister Anderson');
        assert.equal(promo.custom.displayCMOnCart, true);
    });
});
