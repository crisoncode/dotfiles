...
function showProductPage(querystring, res) {
    ...
    if (pdpRedesignEnabled) {
        ...
        if (product.oneSize) {
            product.ats = ProductMgr.getProduct(product.id).availabilityModel.getInventoryRecord().getATS().getValue();
        }
    }
...
server.append('Variation', cache.applyShortPromotionSensitiveCache, function (req, res, next) {
    ...
    if (product) {
        ...
        const stockContext = { product: { ats: ProductMgr.getProduct(product.id).availabilityModel.getInventoryRecord().getATS().getValue() } };
