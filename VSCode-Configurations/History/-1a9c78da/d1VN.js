...
server.append('Variation', cache.applyShortPromotionSensitiveCache, function (req, res, next) {
    ...
    if (product) {
        ...
        product.deliveryRedesignHtml = renderTemplateHelper.getRenderedHtml(deliveryContext, 'product/productDetailRedesignComponents/stock');
...
server.get('GetShippingCosts', function (req, res, next) {
    var qs = req.querystring;
    var taxRate = qs.taxRate;
    var freeShipping = qs.freeShipping === 'true' || false;

    // Set applicable Pricebooks
    priceHelper.setApplicablePricebooks();

    res.render('product/components/shippingCosts', {
        taxRate: taxRate,
