...
const badges = require('*/cartridge/scripts/helpers/badges');
...
function showProductPage(querystring, res) {
    ...
    const pdpRedesignEnabled = false;
...
server.append(
    ...
    function (req, res, next) {
        ...
        const Redesigns = require('*/cartridge/scripts/redesigns');
        let isProductPageRedesign = false;

        if (Redesigns.isABTestEnabled(res) || Redesigns.isABTestFeatureEnabled('productTiles', res)) {
            isProductPageRedesign = true;
        }
        ...
        res.setViewData({
            ...
            badges: (isProductPageRedesign) ? badges.getBadges(product, 2) : false,
...
/**
 * Renders the Product Stock section
 * @param {Object} querystring - query string parameters
 * @param {Object} res - response object
 */
server.get('StockMessage', function (req, res, next) {
    const pid = req.querystring.pid;
    const product = ProductFactory.get({ pid: pid });

    res.render('product/productDetailRedesignComponents/stockMessage', {
        product: {
            id: product.id,
...
server.append('Variation', cache.applyShortPromotionSensitiveCache, function (req, res, next) {
    ...
    if (product) {
        ...
        if (product.tieredPrices) {
         ...
         // append HTML for stock message
        const stockContext = { product: {
            id: product.id,
            expectedDeliveryDate: product.expectedDeliveryDate,
            ats: product.ats }
        };
        product.stockHtml = renderTemplateHelper.getRenderedHtml(stockContext, 'product/productDetailRedesignComponents/stockMessage');
        ...
        product.shippingRedesignHtml = renderTemplateHelper.getRenderedHtml(deliveryContext, 'product/productDetailRedesignComponents/standardShippingPre');
