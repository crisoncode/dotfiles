...
const configurations = {
    ...
    stockMessageContainer: '.js-stock-message-container',
...
/**
 * Paint the Stock label information for the selected variation product
 * @param {Object} product Contains all the product info getted by the Ajax call
 */
function paintStock(product) {
    $(configurations.stockMessageContainer).html(product.stockHtml);
}
...
const initSizeSelectorEvent = () => {
    ...
    if ($sizesDropdown.length > 0) {
        $(configurations.sizes.eventListenerSelector).on('click', { buildVariationChoiceAjaxCall: buildVariationChoiceAjaxCall() }, function (event) {
            event.preventDefault();
            event.stopPropagation();

            event.data.buildVariationChoiceAjaxCall($(this));
