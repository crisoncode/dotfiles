 ...
 * @param {String} attributeValue contains the current value without enhancements
 * @return {string|boolean} returns an string if an attribute will be enhanced, false if not
 */
function enhanceValuesWithMetrics(attribute, attributeValue) {
    let Resource = require('dw/web/Resource');
    let value = false;

    if (attribute.key === 'weight') {
        value = Resource.msgf('text.weightunstrung.unit', 'product', null, attributeValue);
    }

    if (attribute.key === 'headSize') {
        value = Resource.msgf('text.headsize.unit', 'product', null, attributeValue);
    }

    if (attribute.key === 'balance') {
        value = Resource.msgf('product.enhancement.balancePoint', 'product', null, attributeValue);
    }

    if (attribute.key === 'length') {
        value = Resource.msgf('product.enhancement.length', 'product', null, attributeValue);
    }
    return value;
}
...
/**
 *
 * Method for get all the custom attributes values that will be highlighted.
 * @param {Object} args contains all the arguments.
 * @param {Object} args.attribute contains all the info mapped about an attribute that will be highlighted
 * @param {Object} args.productCustomAttributes contains all the custom attrs getted by the dw.catalog.Product object.
 * @returns {string} with the value of the attribute.
 */
function getProductAttributeValue(args) {
    let attributeValue = args.productCustomAttributes[args.attribute.attributeID];
    let enhancedValue = enhanceValuesWithMetrics(args.attribute, attributeValue);
    if (enhancedValue) {
        attributeValue = enhancedValue;
    }
...
function fillAttributes(args) {
    ...
    Object.keys(args.attributesMapping).forEach((key) => {
        ...
        if (attribute.enabled && attribute.attributeID &&
            Object.prototype.hasOwnProperty.call(productCustomAttributes, attribute.attributeID)) {
            attribute.key = key;
            attribute.value = getProductAttributeValue({ attribute: attribute, productCustomAttributes: productCustomAttributes });
            attributesFilledResult.push(attribute);
        }
    });
