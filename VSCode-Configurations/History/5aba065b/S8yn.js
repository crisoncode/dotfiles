...
function handleVariantResponse(response, $productContainer, selectedLocale) {
    ...
    // NOT NECESSARY  // Update primary images
    ...
    // IMPORTANT, CAMBIO DE PRICES    // Update pricing, unit-price & tiered-price info
    ...
    // RANGOS DE PRECIOS SEGUN BONUS    // Update tiered price table
    if (!isChoiceOfBonusProducts) {
    ...
    selectAttribute: function () {
        ...
        $(document).on('click', '.size-tiles .size:not(.sold-out)', function (e) {
            ...
            if ($(e.currentTarget).hasClass('selected') === false) {
                // avoid to refresh the content when selecting a size to check the store inventory
                if (!$(e.currentTarget).parents('#storeInventoryModal').length) {
