...
const initSizeSelectorEvent = () => {
    ...
    if ($sizesSelectors.length > 0) {
        $(configurations.racketSize.eventListenerSelector).on('click', { variationsSelector: variationsSelector, test: 'cris' }, function (event) {
            ...
            event.data.variationsSelector.buildVariationChoiceAjaxCall(this);
        });
    }
};
