...
server.get(
    ...
    function (req, res, next) {
        ...
        basketModel.addToCartUrl = URLUtils.url('Cart-AddProduct').toString();
        ...
        if (currentBasket) {
            Transaction.wrap(function () {
                ...
                data.bonusProducts.forEach(function (bonusProduct) {
                    ...
                    if (product.variationGroup && bonusProduct.selection) {
                        var attributes = {
                            pid: product.ID,
                            variables: {}
                        };
                        var ProductFactory = require('*/cartridge/scripts/factories/product');
                        attributes.variables[bonusProduct.selection[0].attrId] = { 'id': product.ID, value: bonusProduct.selection[0].attrVal};
                        product = ProductFactory.get(attributes);
