...
function getAllAttrValues(
    ...
    return collections.map(attrValues, function (value) {
        ...
        if (attr.ID === 'size') {
            var localizedStrings = stringUtil.getLocalizedStrings(value, ['de_DE', 'en_GB', 'en_US']);
            ...
            var colorVariationAttribute = variationModel.getProductVariationAttribute(ATTRIBUTE_NAME_COLOR);
            var currentColor = variationModel.getSelectedValue(colorVariationAttribute);
            if (currentColor) {
                var filter = new HashMap();
                filter.put(attr.ID, value.value);
                filter.put('color', currentColor.value);

                var filteredVariants = variationModel.getVariants(filter);
